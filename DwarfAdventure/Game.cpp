...
Game::Game(sf::RenderWindow& window,int const frameLimit, Player& heros)
{
	...
	while (window.isOpen())
	{
		...
		//Test des collisions avec le décor
		...
		//Test des collisions après le déplacement
		for (sf::IntRect tileRect : levelCollision)
		{
			if (herosRect.intersects(sf::FloatRect(tileRect)))
			{
				heros.move(-movement); //on annule le déplacement
				heros.changeAnimation("stop");
				break;
			}
		}

		level.setPosition(sf::Vector2f((float)(vm.width / 2 - 32), (float)(vm.height / 2 - 32)) - heros.getPos()); //On déplace le niveau
